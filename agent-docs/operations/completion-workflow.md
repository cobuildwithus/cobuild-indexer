...
Non-docs rule: for changes touching production code or tests, the two audit passes below are mandatory before final handoff.
...
3. Re-run required checks after the simplify pass.
4. Run a completion audit using `agent-docs/prompts/task-finish-review.md` with full change context. Expect about 5 to 10 minutes on non-trivial diffs; do not rush it or cancel it early just because it has not answered in the first minute.
5. Treat the final completion audit as the remaining coverage/proof-gap review too. If it finds meaningful missing tests or boundary-level verification, add the smallest high-impact proof before handoff instead of creating a separate coverage-audit pass.
6. Final handoff must report required-check results; green required checks remain the default completion bar.
7. If a required check fails for a credibly unrelated pre-existing reason, commit your exact touched files and hand off with the failing command, failing target, and why your diff did not cause it. If you cannot defend that separation, treat the failure as blocking.
...
- During simplify/completion-audit passes, never overwrite, discard, or revert existing worktree edits.
...
- Prefer a fresh subagent for the final completion audit; there is no separate required coverage-audit subagent.
- Prefer a patient wait window over repeated short polling for simplify and final-review subagents; a realistic default is 5 to 10 minutes for each audit pass on medium or large diffs.
