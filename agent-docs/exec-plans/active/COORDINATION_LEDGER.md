...
| --- | --- | --- | --- | --- | --- |
| codex-gpt5-keeper-outbox-bridge-2026-03-04 | Add keeper outbox projection writes in lockstep with `protocol_event` so keepers can consume event-driven intents from indexer DB | `ponder.schema.ts`, `src/helpers/protocolEvent.ts`, `ARCHITECTURE.md`, `agent-docs/references/{schema-relations-map,event-handler-map}.md`, `agent-docs/exec-plans/active/COORDINATION_LEDGER.md` | add `keeperOutbox` table; add deterministic `keeperOutboxId` helper; add outbox insert path in `insertProtocolEvent` | Keep writes replay-safe/idempotent and avoid `context.db.sql`; outbox ordering must be deterministic for external router cursors | 2026-03-04 |
