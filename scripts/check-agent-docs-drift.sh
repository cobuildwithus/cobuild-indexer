...
if has_change '^(src/|scripts/|ponder\.config\.ts$|ponder\.schema\.ts$|wagmi\.config\.ts$|abis\.ts$|addresses(\..+)?\.ts$|AGENTS\.md$|ARCHITECTURE\.md$|README\.md$|package\.json$|tsconfig\.json$|\.github/workflows/(test|doc-gardening)\.yml$)'; then
  code_changed=1
fi
